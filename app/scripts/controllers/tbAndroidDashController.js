'use strict';
 ...
 .controller('tbAndroidDashCtrl', function ($scope,$state,QualityService,chartService) {
    ...
    $scope.dropboxitemselected = function(item){
        ...
        var result = QualityService.get({platform:'tbandroid',method:'score',version:item.id},function(data){
            $scope.init(data);
    ...
    $scope.getData = function(){
        var result = QualityService.get({platform:'tbandroid',method:'versions'},function(data){
        $scope.versions = data.data;
        $scope.selectedVersion = $scope.versions[0].name;
        $scope.selectedVersionId = $scope.versions[0].id;
        var summary = QualityService.get({platform:'tbandroid',method:'score',version:$scope.selectedVersionId},function(data){
            $scope.init(data);
            $scope.drawPerf(data.data);
            $scope.drawScore(data.data);
            $scope.drawCrash(data.data);
        ...
        $scope.scoreChartConfig.options.plotOptions.series.point.events.click = function(events){
            for (var i = data.score_trends.detail.length - 1; i >= 0; i--) {
                if (data.score_trends.detail[i].version === events.point.category){
                    $state.go("dashboard.tbandroid.detail",{versionId:data.score_trends.detail[i].project_id,from:'product_quality'},{reload:true})
                }
            };
        }
